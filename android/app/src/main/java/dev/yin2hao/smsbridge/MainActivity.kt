...
class MainActivity : AppCompatActivity() {
    ...
    private fun save(enable: Boolean) { settings.controllerUrl = url.text.toString(); settings.token = token.text.toString(); settings.enabled = enable; settings.rootFallback = when (findViewById<Spinner>(R.id.mode).selectedItemPosition) { 0 -> RootManager.isRootAvailable(); 2 -> true; else -> false }; if (enable) { RootManager.setWatchdogEnabled(true); RootManager.installWatchdog(); SmsMonitorService.start(this) } else RootManager.setWatchdogEnabled(false); refreshStatus() }
